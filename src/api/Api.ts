...
class Api {
    ...
    async fetchData() {
        this.modifications = await fetch('/data/data.json').then(res => res.json()).then(result => result)
    }
    ...
    getModificationsImg(): Promise<{id: number, img: string}[]> {
        return new Promise((resolve) => {
            if (this.modifications.length === 0) {
                this.fetchData().then(()=>{
                    resolve(this.modifications.map((modification) => ({id: modification.id, img: modification.img.preview})))
                })
            } else resolve(this.modifications.map((modification) => ({id: modification.id, img: modification.img.preview})))
        })
    }
}
